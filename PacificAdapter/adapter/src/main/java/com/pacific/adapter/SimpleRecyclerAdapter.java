...
public class SimpleRecyclerAdapter<T extends RecyclerItem> extends Adapter<SimpleRecyclerViewHolder> implements DataIO<T> {
    ...
    public SimpleRecyclerViewHolder onCreateViewHolder(ViewGroup parent, int viewType) {
        ...
        return new SimpleRecyclerViewHolder(inflater.inflate(get(flagPosition).getLayout(), parent, false)) {
        };
    }

    @Override
    public void onViewRecycled(SimpleRecyclerViewHolder holder) {
        int position = holder.getAdapterPosition();
        if (position != NO_POSITION) {
            SimpleRecyclerViewHolder helper = (SimpleRecyclerViewHolder) holder.itemView.getTag(R.integer.adapter_view_holder);
    ...
    @Override
    public void onViewAttachedToWindow(SimpleRecyclerViewHolder holder) {
        int position = holder.getAdapterPosition();
        if (position != NO_POSITION) {
            SimpleRecyclerViewHolder helper = (SimpleRecyclerViewHolder) holder.itemView.getTag(R.integer.adapter_view_holder);
    ...
    @Override
    public void onViewDetachedFromWindow(SimpleRecyclerViewHolder holder) {
        int position = holder.getAdapterPosition();
        if (position != NO_POSITION) {
            SimpleRecyclerViewHolder helper = (SimpleRecyclerViewHolder) holder.itemView.getTag(R.integer.adapter_view_holder);
    ...
    @Override
    public boolean onFailedToRecycleView(SimpleRecyclerViewHolder holder) {
        int position = holder.getAdapterPosition();
        if (position != NO_POSITION) {
